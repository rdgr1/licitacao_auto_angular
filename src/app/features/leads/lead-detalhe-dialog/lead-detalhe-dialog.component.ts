...
const STATUS_META: Record<LeadStatus, { label: string; bg: string; color: string; dot: string }> = {
  NOVO:            { label: 'Novo',            bg: '#EFF6FF', color: '#1E40AF', dot: '#3B82F6' },
  EM_TRIAGEM:      { label: 'Em Triagem',       bg: '#F5F3FF', color: '#5B21B6', dot: '#8B5CF6' },
  VERIFICANDO_REQ: { label: 'Verificando Req.', bg: '#FFFBEB', color: '#92400E', dot: '#F59E0B' },
  QUALIFICADO:     { label: 'Qualificado',      bg: '#F0FDF4', color: '#166534', dot: '#10B981' },
  DESCARTADO:      { label: 'Descartado',       bg: '#F8FAFC', color: '#64748B', dot: '#94A3B8' },
};
...
@Component({
  ...
  template: `
    <div class="ld-shell">
      ...
      <div class="ld-actions">
        ...
        <div class="act-right">
          @if (data.status !== 'EM_TRIAGEM' && data.status !== 'QUALIFICADO' && data.status !== 'DESCARTADO') {
            <button class="act-btn act-triagem" (click)="atualizar('EM_TRIAGEM')" [disabled]="salvando">
              <mat-icon>manage_search</mat-icon>Em Triagem
            </button>
          }
          @if (data.status === 'EM_TRIAGEM') {
            <button class="act-btn act-verif" (click)="atualizar('VERIFICANDO_REQ')" [disabled]="salvando">
              <mat-icon>checklist</mat-icon>Verificar Req.
            </button>
          }
          @if (data.status !== 'DESCARTADO') {
            <button class="act-btn act-discard" (click)="atualizar('DESCARTADO')" [disabled]="salvando">
              <mat-icon>block</mat-icon>
            </button>
          }
          @if (data.status !== 'QUALIFICADO' && data.status !== 'DESCARTADO') {
            <button class="act-btn act-qualify" (click)="atualizar('QUALIFICADO')" [disabled]="salvando">
              @if (salvando) { <span class="saving-dot"></span> } @else { <mat-icon>verified</mat-icon> }
              Qualificar
            </button>
          }
          @if (data.status === 'QUALIFICADO') {
            <div class="act-done"><mat-icon>check_circle</mat-icon>Qualificado</div>
          }
        </div>
      </div>
    </div>
  ...
  styles: [`
    ...
    .close-btn { width: 30px; height: 30px; mat-icon { font-size: 18px; color: #94A3B8; } }
    ...
    .act-right { display: flex; align-items: center; gap: 6px; }
    .act-btn { display: inline-flex; align-items: center; gap: 5px; padding: 7px 14px; border-radius: 8px; border: 1.5px solid transparent; font-size: 12.5px; font-weight: 600; font-family: inherit; cursor: pointer; transition: all 130ms;
      ...
      &.act-triagem { background: #F5F3FF; border-color: #DDD6FE; color: #5B21B6; &:hover { background: #EDE9FE; } }
      &.act-verif   { background: #FFFBEB; border-color: #FDE68A; color: #92400E; &:hover { background: #FEF3C7; } }
      &.act-discard { background: transparent; border-color: #E2E8F0; color: #94A3B8; padding: 7px 10px; &:hover { background: #FEE2E2; border-color: #FCA5A5; color: #EF4444; } }
      &.act-qualify { background: #11BF7F; border-color: #11BF7F; color: #fff; box-shadow: 0 2px 8px rgba(17,191,127,0.25); &:hover { background: #0DA66E; } } }
...
export class LeadDetalheDialogComponent implements OnInit {
  ...
  atualizar(status: LeadStatus): void {
    this.salvando = true;
    this.leadService.atualizarStatus(this.data.uuid, { status, revisadoPor: 'analista@brasfort.com.br' }).subscribe({
      next: () => { this.toast.success(STATUS_META[status].label); this.dialogRef.close(true); },
