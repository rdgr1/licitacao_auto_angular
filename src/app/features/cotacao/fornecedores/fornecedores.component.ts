        ...
        @if (loading()) {
          ...
          <div class="table-state">
            ...
            @if (!searchQuery() && !selectedCat()) {
              <button mat-stroked-button (click)="openForm(null)">
              ...
              <td mat-cell *matCellDef="let row">
                <div class="actions-cell">
                  ...
                  <button
                    ...
                    matTooltip="Excluir"
                    color="warn"
                    (click)="confirmarExclusao(row); $event.stopPropagation()"
                  >
...
export class FornecedoresComponent implements OnInit {
  ...
  filtered = computed(() => {
    ...
    if (q)
      list = list.filter(
        (f) => f.nome.toLowerCase().includes(q) || f.email.toLowerCase().includes(q),
      );
  ...
  load(): void {
    ...
    this.svc.listarFornecedores({ page: this.currentPage(), size: this.pageSize }).subscribe({
      next: (p) => {
        this.fornecedores.set(p.content ?? []);
        this.totalElements.set(p.totalElements);
        this.loading.set(false);
      },
      error: () => {
        this.toast.error('Erro ao carregar fornecedores');
        this.loading.set(false);
      },
    });
  }
