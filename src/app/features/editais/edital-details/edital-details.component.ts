...
export class EditalDetailsComponent implements OnInit {
  ...
  private toggleSection<T>(sig: WritableSignal<Section<T>>, loader: () => Observable<T[]>): void {
    const s = sig();
    if (s.open) {
      sig.update((x) => ({ ...x, open: false }));
      return;
    }
  ...
  private retrySection<T>(sig: WritableSignal<Section<T>>, loader: () => Observable<T[]>): void {
  ...
  toggleItens(): void {
    ...
    if (s.open) {
      this.itensSection.update((x) => ({ ...x, open: false }));
      return;
    }
      ...
      next: (data) => {
        if (data.length > 0) {
          ...
          this.editaisService.sincronizarItens(id).subscribe({
            next: (synced) =>
              this.itensSection.update((x) => ({
                ...x,
                data: synced,
                state: synced.length ? 'loaded' : 'empty',
              })),
  ...
  toggleArquivos(): void {
    this.toggleSection(this.arquivosSection, () =>
      this.editaisService.getArquivos(this.edital()!.id),
    );
  }
  toggleContratos(): void {
    this.toggleSection(this.contratosSection, () =>
      this.editaisService.getContratos(this.edital()!.id),
    );
  }
  toggleAtas(): void {
    this.toggleSection(this.atasSection, () => this.editaisService.getAtas(this.edital()!.id));
  }
  toggleHistorico(): void {
    this.toggleSection(this.historicoSection, () =>
      this.editaisService.getHistorico(this.edital()!.id),
    );
  }

  retryItens(): void {
    this.itensSection.update((x) => ({ ...x, state: 'idle', open: false }));
    this.toggleItens();
  }
  retryArquivos(): void {
    this.retrySection(this.arquivosSection, () =>
      this.editaisService.getArquivos(this.edital()!.id),
    );
  }
  retryContratos(): void {
    this.retrySection(this.contratosSection, () =>
      this.editaisService.getContratos(this.edital()!.id),
    );
  }
  retryAtas(): void {
    this.retrySection(this.atasSection, () => this.editaisService.getAtas(this.edital()!.id));
  }
  retryHistorico(): void {
    this.retrySection(this.historicoSection, () =>
      this.editaisService.getHistorico(this.edital()!.id),
    );
  }
  ...
  reprocessar(): void {
    ...
    this.editaisService.reprocessar(id).subscribe({
      ...
      error: () => {
        this.toast.error('Erro ao reprocessar');
        this.reprocessando.set(false);
      },
    });
  }
  ...
  delete(): void {
    ...
    ref.afterClosed().subscribe((confirmed) => {
      ...
      this.editaisService.delete(e.id).subscribe({
        next: () => {
          this.toast.success(`Edital ${e.numero} excluído`);
          this.router.navigate(['/editais']);
        },
