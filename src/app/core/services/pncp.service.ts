...
import { PncpModalidade, PncpUf } from '../models/pncp.model';
...
export class PncpService {
  ...
  createModalidade(payload: { codigo: number; nome: string; ativo: boolean }): Observable<PncpModalidade> {
    return this.http.post<PncpModalidade>(`${this.base}/modalidades`, payload);
  }

  updateModalidade(uuid: string, payload: { codigo: number; nome: string; ativo: boolean }): Observable<PncpModalidade> {
